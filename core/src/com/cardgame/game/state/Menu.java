...
public class Menu extends GameState {
	...
	public void handleInput(float dt){
		if(Gdx.input.isKeyJustPressed(Input.Keys.SPACE)){
			switch(select){
				case 0: gsm.popState();
						gsm.pushState(GameStateManager.SERVER); break;
				case 1: gsm.popState();
						String address = JOptionPane.showInputDialog(new JFrame(), "Address of Server?");
						gsm.pushClient(GameStateManager.CLIENT,address); break;
				case 2: gsm.pushState(GameStateManager.ABOUT); break;
				case 3: gsm.pushState(GameStateManager.OPTIONS); break;
...
////			x++;
//		}
		if(select<0) select +=NUMOPTIONS;
		else select%=NUMOPTIONS;
	}
...
//		sb.draw(new Texture("MainMenu.png"),0,0);
		switch(select){
			case 0: titleFont.draw(sb, "Start Server", 320-4*32+5, 240-5); break;// sb.draw(Sprite.getSprite(Sprite.sprites, 120), 195,201); break;
			case 1: titleFont.draw(sb, "Connect as Client", 320-4*32+5, 240-5); break;// sb.draw(Sprite.getSprite(Sprite.sprites, 120), 197,184); break;
			case 2: titleFont.draw(sb, "About", 320-4*32+5, 240-5); break;// sb.draw(Sprite.getSprite(Sprite.sprites, 120), 197,184); break;
			case 3: titleFont.draw(sb, "Options", 320-4*32+5, 240-5); break;// sb.draw(Sprite.getSprite(Sprite.sprites, 120), 197,184); break;
			case 4: titleFont.draw(sb, "Exit", 320-4*32+5, 240-5); break;// sb.draw(Sprite.getSprite(Sprite.sprites, 120), 199,167); break;
		}
